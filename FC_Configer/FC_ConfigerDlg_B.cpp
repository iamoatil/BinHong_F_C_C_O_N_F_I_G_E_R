...
BOOL CFC_ConfigerDlg::Verify_FC_Ethernet(void)
{
	...
	for(int i=0;i<count;i++)
	{
		...
		for(int j=i+1;j<count;j++)
		{
			...
			{
				CString err;
				err.Format(_T("第%d行与第%d行的DID重复!"),j+1,i+1);
...
{	//显示表格转换为数组
	...
	{
		//DID
		CString strItem = m_UGGridDID_Eth.GetItemText(i,1);
		BYTE did = _ttoi(strItem);
		//网口
		strItem = m_UGGridDID_Eth.GetItemText(i,2);
		...
		port += (portB<<4);
		Temp_Map.Add(CPoint((int)(did),(int)port));
	}
	//更新转发表对应的数据
	FC_DID_ETH_Map.RemoveAll();
	for(int i=0;i<Temp_Map.GetCount();i++)
