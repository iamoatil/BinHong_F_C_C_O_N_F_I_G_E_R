...

{
	...
	m_crOnFG=RGB(0,255,0);

	m_crOffFG=RGB(255,0,0);
...

{
	...
	if(m_bOn)
		...
		pdc.TextOut(r.CenterPoint().x-(int)(m_sOnString.GetLength()*3.5),r.CenterPoint().y-10,m_sOnString,m_sOnString.GetLength());

	else

		pdc.TextOut(r.CenterPoint().x-(int)(m_sOffString.GetLength()*3.5),r.CenterPoint().y-10,m_sOffString,m_sOffString.GetLength());
